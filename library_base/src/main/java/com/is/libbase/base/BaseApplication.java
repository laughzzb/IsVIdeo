...
//        }, 2000);
    ...
    private void initUmeng() {
        SharedPreferences preferences =getSharedPreferences("agreementStatus", Context.MODE_PRIVATE);
        boolean privacy = preferences.getBoolean("agreement privacy", false);//没有存的话默认为false
        Log.i(TAG, "initUmeng: privacy" +privacy);
        if (privacy) {
            new Thread(new Runnable() {
                @Override
                public void run() {
                    UmengHepler.init(BaseApplication.this);
                }
            }).start();
        }
    }
